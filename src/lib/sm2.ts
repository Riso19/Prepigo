 ...
 */
export const sm2 = (quality: Sm2Quality, params: Sm2Parameters, cardState?: Partial<Sm2State>): Sm2State => {
  const repetitions = cardState?.repetitions ?? 0;
  const easinessFactor = cardState?.easinessFactor ?? params.initialEasinessFactor;
  const interval = cardState?.interval ?? 0;

  if (quality < 3) {
    // Incorrect response: reset progress
    const nextRepetitions = 0;
    const nextInterval = 1; // Always reset to 1 day on failure
  ...
  if (repetitions === 0) {
    nextInterval = params.firstInterval;
  } else if (repetitions === 1) {
    nextInterval = params.secondInterval;
  } else {
  ...
  const nextEasinessFactor = Math.max(params.minEasinessFactor, newEasinessFactor);
