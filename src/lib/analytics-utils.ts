...
export const calculateDueStats = (items: (FlashcardData | McqData)[], settings: SrsSettings) => {
  ...
  items.forEach((item) => {
    ...
    // For MCQs ('question' in item), force FSRS when global scheduler is SM-2
    const scheduler =
      'question' in item
...
export const calculateRetentionDistribution = (
...
export const calculateForecast = (items: (FlashcardData | McqData)[], settings: SrsSettings) => {
  ...
  items.forEach((item) => {
    ...
    if (srsData) {
      const isFsrs = (d: unknown): d is { state: State } =>
        typeof (d as { state?: unknown })?.state === 'number';
      const include = !isFsrs(srsData) || (isFsrs(srsData) && srsData.state !== State.New);
      if (include) {
        const dueDate = startOfDay(new Date((srsData as { due: string }).due));
...
export const calculateAverageRetention = (
  items: (FlashcardData | McqData)[],
  settings: SrsSettings,
) => {
  const scheduler = settings.scheduler;
...
export const calculateAtRiskItems = (
  items: (FlashcardData | McqData)[],
  settings: SrsSettings,
  riskThreshold = 0.4,
) => {
...
export const calculateSuspectedGuesses = (logs: (ReviewLog | McqReviewLog)[]) => {
  ...
  const stdDev = Math.sqrt(
    durations.map((x) => Math.pow(x - avgDuration, 2)).reduce((a, b) => a + b) / durations.length,
  );
...
export const calculateMemoryDecayVelocity = (
...
export const calculateAverageKnowledgeHalfLife = (
  items: (FlashcardData | McqData)[],
  settings: SrsSettings,
) => {
  const scheduler = settings.scheduler;
...
export const calculateOverlearningRatio = (
  logs: (ReviewLog | McqReviewLog)[],
  settings: SrsSettings,
) => {
  const scheduler = settings.scheduler;
...
export const calculateReviewTimeDistribution = (logs: (ReviewLog | McqReviewLog)[]) => {
  const durations = logs
    ...
    .map((log) => log.duration! / 1000); // in seconds
...
export const calculateDailySummary = (logs: (ReviewLog | McqReviewLog)[]) => {
  ...
  const summaryByDay = new Map<
    string,
    { reviews: number; correct: number; totalDifficulty: number; difficultyCount: number }
  >();
  ...
  return Array.from(summaryByDay.entries())
    .map(([date, data]) => ({
      ...
      avgDifficulty:
        data.difficultyCount > 0 ? (data.totalDifficulty / data.difficultyCount) * 100 : 0, // as percentage
    }))
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
};
