...
import { ArrowLeft, ChevronLeft, ChevronRight, FileText, Plus, Highlighter, StickyNote, Copy, Paintbrush, List, Trash2 } from "lucide-react";
import { Switch } from "@/components/ui/switch";
import FlashcardFromPdfDialog, { type PdfSelection } from "@/components/resources/FlashcardFromPdfDialog";
...
export default function ResourceViewerPage() {
  ...
  const [textItems, setTextItems] = useState<Array<{
    str: string;
    x: number; y: number; w: number; h: number; // page space (scale 1)
  }>>([]);
  ...
  const [occlusionSeed, setOcclusionSeed] = useState<{ imageUrl: string; masks?: { x: number; y: number; width: number; height: number }[] } | null>(null);
    ...
    (async () => {
      ...
      try {
        ...
        for (const it of textContent.items) {
          ...
          const tf: number[] = (it as any).transform; // [a,b,c,d,e,f]
          const x = tf[4];
          const yTop = tf[5];
          const h = Math.abs(tf[3]);
          const w: number = (it as any).width; // in page units
  ...
  const onMouseUp = async () => {
    ...
    if (viewport) {
      ...
      text = viewportItems.filter(v => overlaps(v, selV)).map(v => v.str).join(" ");
    }
    const pageRect = { x: dragRect.x / scale, y: dragRect.y / scale, w: dragRect.w / scale, h: dragRect.h / scale };
  ...
  const onHighlightClick = async (e: React.MouseEvent, h: ResourceHighlight) => {
    if (e.shiftKey) {
  ...
  const getSelectionImageDataUrl = (): { dataUrl: string; cropX: number; cropY: number; cropW: number; cropH: number } | null => {
    const canvas = canvasRef.current;
  ...
  const onCreateCard = async (payload: { deckId: string; front: string; back: string; linkHighlight: boolean; cardType: 'basic'|'imageOcclusion'; masks?: { x: number; y: number; width: number; height: number }[] }) => {
    if (!resource) return;
    let newCard: any;
    ...
    const addToDeck = (ds: typeof decks): typeof decks => {
      const addRec = (d: any): any => {
    ...
    if (payload.linkHighlight && currentHighlightId) {
      ...
      setHighlights(prev => prev.map(h => h.id === currentHighlightId ? { ...h, linkedCardId: newCard.id } : h));
      ...
      <div className="mx-auto w-full max-w-5xl px-3 sm:px-4 py-3 sm:py-4">
        ...
        <Card className="p-3 sm:p-4 flex flex-col min-h-[70vh]">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
            ...
            <Button variant="outline" size="sm" onClick={() => canPrev && setPageNum(p => Math.max(1, p - 1))} disabled={!canPrev}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => canNext && setPageNum(p => Math.min(pageCount, p + 1))} disabled={!canNext}>
              <ChevronRight className="h-4 w-4" />
            </Button>
            <Separator orientation="vertical" className="h-6" />
            <div className="flex items-center gap-1 text-xs">
              <Button variant="outline" size="icon" className="h-7 w-7" onClick={()=> setScale(s => Math.max(0.5, +(s - 0.25).toFixed(2)))}>-</Button>
              <div className="w-12 text-center tabular-nums">{Math.round(scale*100)}%</div>
              <Button variant="outline" size="icon" className="h-7 w-7" onClick={()=> setScale(s => Math.min(3, +(s + 0.25).toFixed(2)))}>+</Button>
            </div>
            <Separator orientation="vertical" className="h-6" />
            <Button size="sm" className="gap-2" onClick={() => { setSelection({ text: "", page: pageNum }); setDialogOpen(true); }}>
              <Plus className="h-4 w-4" /> Create flashcard
            </Button>
            <Sheet open={showHighlights} onOpenChange={setShowHighlights}>
              <SheetTrigger asChild>
                <Button variant="outline" size="sm" className="gap-2"><List className="h-4 w-4"/> Highlights</Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-full sm:w-[420px] flex flex-col">
                ...
                <div className="mt-4 space-y-3 overflow-y-auto max-h-[calc(100vh-8rem)] pr-1">
                  {highlights.length === 0 && (
                    <div className="text-sm text-muted-foreground">No highlights yet. Drag on the page to create one.</div>
                  )}
                        ...
                        <div className="flex items-center gap-1">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" title="Set color"><Paintbrush className="h-4 w-4"/></Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={()=>setColorForHighlight(h.id,'yellow')}>Yellow</DropdownMenuItem>
                              <DropdownMenuItem onClick={()=>setColorForHighlight(h.id,'green')}>Green</DropdownMenuItem>
                              <DropdownMenuItem onClick={()=>setColorForHighlight(h.id,'blue')}>Blue</DropdownMenuItem>
                              <DropdownMenuItem onClick={()=>setColorForHighlight(h.id,'pink')}>Pink</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                          <Button variant="ghost" size="icon" className="h-7 w-7" title="Add note" onClick={()=>openNoteEditor(h)}>
                            <StickyNote className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600" title="Delete" onClick={async ()=>{
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7" title="Go to" onClick={()=>{ setPageNum(h.page); }}>
                            <ChevronRight className="h-4 w-4 rotate-180"/>
                          </Button>
                        </div>
                      </div>
                      {h.text && <div className="mt-1 text-sm">{h.text}</div>}
                      {h.note && <div className="mt-1 text-xs text-muted-foreground">Note: {h.note}</div>}
                    </div>
                  ))}
                </div>
                {noteForId && (
                  <div className="mt-4 border-t pt-3">
                    ...
                    <Textarea value={noteDraft} onChange={(e)=>setNoteDraft(e.target.value)} rows={4} placeholder="Add a note"/>
                    <div className="mt-2 flex gap-2 justify-end">
                      <Button variant="ghost" size="sm" onClick={()=>{ setNoteForId(null); setNoteDraft(""); }}>Cancel</Button>
        ...
        <div className="mt-2 flex items-center gap-3">
          <div className="h-2 grow rounded bg-muted">
            <div className="h-2 rounded bg-primary" style={{ width: `${(pageNum / Math.max(1, pageCount)) * 100}%` }} />
          </div>
          <div className="text-xs tabular-nums text-muted-foreground">{Math.round((pageNum / Math.max(1, pageCount)) * 100)}%</div>
        </div>

        {/* Inline toolbar for current selection */}
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <Button size="sm" variant="outline" className="gap-2" disabled={!currentHighlightId && !selection} onClick={copySelectionText}>
            <Copy className="h-4 w-4"/> Copy text
          </Button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="sm" variant="outline" className="gap-2" disabled={!currentHighlightId}><Highlighter className="h-4 w-4"/> Color</Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
              <DropdownMenuItem disabled={!currentHighlightId} onClick={()=> currentHighlightId && setColorForHighlight(currentHighlightId,'yellow')}>Yellow</DropdownMenuItem>
              <DropdownMenuItem disabled={!currentHighlightId} onClick={()=> currentHighlightId && setColorForHighlight(currentHighlightId,'green')}>Green</DropdownMenuItem>
              <DropdownMenuItem disabled={!currentHighlightId} onClick={()=> currentHighlightId && setColorForHighlight(currentHighlightId,'blue')}>Blue</DropdownMenuItem>
              <DropdownMenuItem disabled={!currentHighlightId} onClick={()=> currentHighlightId && setColorForHighlight(currentHighlightId,'pink')}>Pink</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          <Button size="sm" variant="outline" className="gap-2" disabled={!currentHighlightId} onClick={()=>{
            const h = highlights.find(x=>x.id===currentHighlightId);
            if (h) { openNoteEditor(h); setShowHighlights(true); }
          }}>
            <StickyNote className="h-4 w-4"/> Note
          </Button>
          <div className="ml-auto flex items-center gap-2 text-xs text-muted-foreground">
            <span>Create card on select</span>
            <Switch checked={createOnSelect} onCheckedChange={setCreateOnSelect} aria-label="Create card on select" />
          </div>
        ...
        <div className="mt-3 relative flex-1 min-h-[50vh] sm:min-h-[60vh] overflow-auto">
          <div className="w-full h-full flex items-center justify-center">
            <div
              ...
              onMouseUp={onMouseUp}
            >
              <canvas ref={canvasRef} className={"bg-muted rounded border block max-w-full h-auto" + (rendering ? " opacity-80" : "")} />
                ...
                h.rects.map((r, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={(e) => { if (e.shiftKey && (e.key === 'Enter' || e.key === ' ')) onHighlightClick(e as any, h); }}
                    />
                  );
                })
              ))}
