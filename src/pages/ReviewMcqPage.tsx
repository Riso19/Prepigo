...
const ReviewMcqPage = () => {
  ...
  const { questionBanks, setQuestionBanks, mcqIntroductionsToday, addIntroducedMcq } =
    useQuestionBanks();
  ...
  useEffect(() => {
    if (bankId === 'custom' && location.state) {
      ...
      const banksToReview =
        bankId && bankId !== 'all'
          ? ([findQuestionBankById(questionBanks, bankId)].filter(Boolean) as QuestionBankData[])
          : questionBanks;
      if (banksToReview.length > 0) {
        const { queue, mcqExamMap: newMcqExamMap } = buildMcqSessionQueue(
          banksToReview,
          questionBanks,
          globalSettings,
          mcqIntroductionsToday,
          exams,
        );
        setSessionQueue(queue);
    ...
    async (rating: Rating) => {
      ...
      const wasNew =
        settings.scheduler === 'fsrs6'
          ? !currentQuestion.srs?.fsrs6 || currentQuestion.srs.fsrs6.state === State.New
          : !currentQuestion.srs?.fsrs || currentQuestion.srs.fsrs.state === State.New;
  ...
  const handleSelectAndSubmit = useCallback(
    ...
    [isSubmitted, currentQuestion, addXp, updateStats, userStats.totalCardsReviewed, updateStreak],
  );
  ...
  const handleRestart = useCallback(() => {
    const banksToReview =
      bankId && bankId !== 'all'
        ...
        : questionBanks;
    if (banksToReview.length > 0) {
      const { queue } = buildMcqSessionQueue(
        banksToReview,
        questionBanks,
        globalSettings,
        mcqIntroductionsToday,
  ...
  const pageTitle =
    bankId === 'custom'
      ? customSessionTitle
      : bankId && bankId !== 'all'
        ? `Reviewing: ${findQuestionBankById(questionBanks, bankId)?.name}`
        : 'Reviewing All Due MCQs';
