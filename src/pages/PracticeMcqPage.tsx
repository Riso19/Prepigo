...
const PracticeMcqPage = () => {
  ...
  const bank = useMemo(
    () =>
      bankId && bankId !== 'all' && bankId !== 'custom'
        ? findQuestionBankById(questionBanks, bankId)
        : null,
    [questionBanks, bankId],
  );
  const currentQuestion = useMemo(
    () => sessionQueue[currentQuestionIndex],
    [sessionQueue, currentQuestionIndex],
  );
  ...
  useEffect(() => {
    if (bankId === 'custom' && location.state?.queue) {
      ...
      if (banksToPractice.length > 0) {
        ...
        const effective = getEffectiveMcqSrsSettings(
          questionBanks,
          bankId || 'all',
          globalSettings,
        );
        let ordered = [...allMcqs];
        // Respect mcqDisplayOrder
        if (effective.mcqDisplayOrder === 'random') {
          ordered = shuffle(ordered);
        } else if (effective.mcqDisplayOrder === 'byTag') {
          ordered.sort((a, b) => (a.tags?.join(',') || '').localeCompare(b.tags?.join(',') || ''));
        } else if (effective.mcqDisplayOrder === 'byDifficulty') {
          // sort ascending by known difficulty (unknown/new last)
          const diff = (m: McqData) => {
            const srs = effective.scheduler === 'fsrs6' ? m.srs?.fsrs6 : m.srs?.fsrs;
            return srs?.difficulty ?? Number.POSITIVE_INFINITY;
          };
          ordered.sort((a, b) => diff(a) - diff(b));
        }
        setSessionQueue(ordered);
      }
    }
  }, [bankId, bank, questionBanks, location.state, questionBanks, globalSettings]);
  ...
  const handleGradeAndProceed = useCallback(
    async (rating: Rating) => {
      ...
      const settings = getEffectiveMcqSrsSettings(questionBanks, bankId || 'all', globalSettings);
      const srsData =
        settings.scheduler === 'fsrs6' ? currentQuestion.srs?.fsrs6 : currentQuestion.srs?.fsrs;
      ...
      void enqueueCriticalSyncOp({
        resource: 'mcqReviewLogs',
        opType: 'create',
        payload: logToSave,
      })
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (isSubmitted && isSrsEnabled && isAnswerCorrect) {
        ...
        if (
          ...
          !isNaN(keyNumber) &&
          keyNumber > 0 &&
          keyNumber <= currentQuestion.options.length
        ) {
  ...
  const pageTitle =
    bankId === 'custom'
      ...
      : bankId === 'all'
        ? 'Practicing All MCQs'
        : `Practicing: ${bank?.name}`;

  // Determine option order per current question (must be before any early returns)
      ...
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4">
        ...
        <Button asChild>
          <Link to="/question-bank">
            <Home className="mr-2 h-4 w-4" /> Go back to Question Banks
          </Link>
        </Button>
      </div>
    );
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-4 pt-4">
              ...
              <Button asChild variant="outline" className="flex-1">
                <Link to="/question-bank">
                  <Home className="mr-2 h-4 w-4" /> Back to Banks
                </Link>
              </Button>
            </div>
          </CardContent>
  ...
  const gradingButtons = [
    {
      label: 'Wrong',
      tooltip: 'Knew you were wrong or guessed wrong',
      grade: 1,
      rating: Rating.Again,
      icon: X,
    ...
    {
      label: 'Unsure',
      tooltip: 'Correct but unsure, or guessed right',
      grade: 2,
      rating: Rating.Hard,
      icon: HelpCircle,
    ...
    {
      label: 'Slow',
      tooltip: 'Correct, but took effort to recall',
      grade: 3,
      rating: Rating.Good,
      icon: Clock,
    ...
    {
      label: 'Easy',
      tooltip: 'Trivial, effortless recall',
      grade: 5,
      rating: Rating.Easy,
      icon: Sparkles,
      ...
      <div className="flex-grow w-full pb-32">
        <div className="relative p-4 sm:p-6 md:p-8">
          ...
          <header className="w-full max-w-2xl mx-auto text-center mb-4 pt-8 sm:pt-0">
            ...
            <Progress
              value={
                sessionQueue.length > 0 ? (currentQuestionIndex / sessionQueue.length) * 100 : 0
              }
              className="mt-4 h-2"
            />
          </header>
        ...
        <div className="w-full max-w-2xl mx-auto p-4">
          {isSubmitted ? (
            isSrsEnabled ? (
              ...
              <div className="h-16 flex items-center justify-center">
                <Button
                  onClick={() => handleGradeAndProceed(Rating.Good)}
