...
const StudyPage = () => {
  ...
  const fsrsInstance = useMemo(() => {
    ...
    const settings = currentDeck
      ? getEffectiveSrsSettings(decks, currentDeck.id, globalSettings)
      : globalSettings;
  ...
  useEffect(() => {
    if (deckId === 'custom' && location.state) {
      ...
      if (decksToStudy.length > 0) {
        const { queue, cardExamMap: newCardExamMap } = buildSessionQueue(
          decksToStudy,
          decks,
          globalSettings,
          introductionsToday,
  ...
  const handleRating = useCallback(
    async (rating: Rating) => {
      ...
      const wasNew =
        currentCard.srs?.fsrs?.reps === 0 ||
        !currentCard.srs?.fsrs ||
        currentCard.srs?.fsrs6?.reps === 0 ||
        !currentCard.srs?.fsrs6 ||
        currentCard.srs?.sm2?.repetitions === 0 ||
        !currentCard.srs?.sm2;
      ...
      if (settings.scheduler === 'fsrs' || settings.scheduler === 'fsrs6') {
        ...
        // SM-2 Logic
        const sm2State = currentCard.srs?.sm2 || {
          state: 'new',
          repetitions: 0,
          lapses: 0,
        ...
        if (rating === Rating.Again) {
          if (isReview) {
            ...
            nextSm2State.easinessFactor = Math.max(
              settings.sm2MinEasinessFactor,
              sm2State.easinessFactor - 0.2,
            );
            const newInterval = Math.max(
              settings.sm2MinimumInterval,
              sm2State.interval * settings.sm2LapsedIntervalMultiplier,
            );
          ...
          const qualityMap: { [key in Rating]: Sm2Quality } = {
            [Rating.Manual]: 0,
            [Rating.Again]: 1,
            [Rating.Hard]: 3,
            [Rating.Good]: 4,
          ...
          if (rating === Rating.Easy) {
            ...
            const steps =
              cardState === 'relearning'
                ? parseSteps(settings.relearningSteps)
                : parseSteps(settings.learningSteps);
            ...
            if (rating === Rating.Hard) {
              ...
              if (nextStep >= steps.length) {
                ...
                const graduationInterval =
                  cardState === 'relearning' ? sm2State.interval : settings.sm2GraduatingInterval;
        ...
        if (
          isReview &&
          rating === Rating.Again &&
          nextSm2State.lapses! >= settings.leechThreshold
        ) {
      ...
      if (currentCard.noteId) {
        ...
        if (cardState === 'new' && settings.buryNewSiblings) {
          ...
          settings.buryInterdayLearningSiblings
        ) {
          shouldBury = true;
        }
      ...
      // Award XP for reviewing a flashcard and update stats
      if (currentCard) {
        addXp('card_reviewed').catch(console.error);
        // Update reviewed count only (study time is handled by useStudyTimer)
        void updateStats({
          totalCardsReviewed: userStats.totalCardsReviewed + 1,
        });
        // Update daily study streak
        void updateStreak('study');
      }
  ...
  useEffect(() => {
    if (isFlipped && currentCard && isSrsEnabled) {
      ...
      if (settings.scheduler === 'fsrs' || settings.scheduler === 'fsrs6') {
        const srsData =
          settings.scheduler === 'fsrs6' ? currentCard.srs?.fsrs6 : currentCard.srs?.fsrs;
  ...
  const pageTitle = isCustomSession
    ? customSessionTitle
    : deckId === 'all'
      ? 'Studying All Due Cards'
      : `Studying: ${findDeckById(decks, deckId || '')?.name}`;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-muted-foreground mb-6">
          You've reviewed all available cards for this session.
        </p>
        <Button asChild>
          <Link to="/">
            <Home className="mr-2 h-4 w-4" /> Go back to My Decks
  ...
  const renderCard = () => {
    ...
    switch (currentCard.type) {
      ...
      case 'imageOcclusion':
        return (
          <ImageOcclusionPlayer
            imageUrl={currentCard.imageUrl}
            occlusions={currentCard.occlusions}
            questionOcclusionId={currentCard.questionOcclusionId}
            description={currentCard.description}
            isFlipped={isFlipped}
            onClick={handleCardClick}
          />
  ...
  const getIntervalText = (rating: Rating) => {
    ...
    if (settings.scheduler === 'sm2' && currentCard) {
      ...
      if (cardState === 'review') {
        ...
        // This is for 'new', 'learning', 'relearning' states
        const steps =
          cardState === 'relearning'
            ? parseSteps(settings.relearningSteps)
            : parseSteps(settings.learningSteps);
    ...
    if (!isSrsEnabled) {
      return (
        <div className="w-full max-w-2xl mx-auto p-4">
          {isFlipped ? (
            <Button onClick={() => handleRating(Rating.Good)} className="w-full h-16 text-lg">
              Next Card
            </Button>
          ) : (
            <Button onClick={() => setIsFlipped(true)} className="w-full h-16 text-lg relative">
              Show Answer
              <span className="absolute bottom-1 right-1 text-xs p-1 bg-black/20 rounded-sm">
                Space
              </span>
            </Button>
          )}
        ...
        {isFlipped ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
            <Button
              ...
              className="relative bg-red-500 hover:bg-red-600 text-white font-bold h-16 text-base flex flex-col"
            >
              <span>Again</span>
              <span className="text-xs font-normal opacity-80">
                {getIntervalText(Rating.Again)}
              </span>
              <span className="absolute bottom-1 right-1 text-xs p-1 bg-black/20 rounded-sm">
                1
              </span>
            ...
            <Button
              ...
              className="relative bg-orange-400 hover:bg-orange-500 text-white font-bold h-16 text-base flex flex-col"
            >
              <span>Hard</span>
            ...
            <Button
              ...
              className="relative bg-green-500 hover:bg-green-600 text-white font-bold h-16 text-base flex flex-col"
            >
              <span>Good</span>
            ...
            <Button
              ...
              className="relative bg-blue-500 hover:bg-blue-600 text-white font-bold h-16 text-base flex flex-col"
            >
              <span>Easy</span>
